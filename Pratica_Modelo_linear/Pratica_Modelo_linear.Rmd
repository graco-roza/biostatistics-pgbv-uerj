...
covariancia <- iris %>% summarise(
  cov = sum((Petal.Length - mean(Petal.Length))*(Petal.Width - mean(Petal.Width))))
...
#Aplico a minha formula para os valores observados de X e comparo o Y predito com o Y real.
...
# Erro padrão  {style="caret-color: rgb(255, 255, 255); color: rgb(255, 255, 255); font-family: Helvetica; font-style: normal; font-variant-caps: normal; letter-spacing: normal; orphans: auto; text-align: start; text-indent: 0px; text-transform: none; white-space: normal; widows: auto; word-spacing: 0px; -webkit-text-size-adjust: auto; -webkit-text-stroke-width: 0px; text-decoration: none;"}
...
$$
Erro = \frac{\sqrt{\frac{\sum_{i=i}^{n}(y_i-\hat{y})^2}{n-2}}}{\sqrt{\sum_{i=1}^{n}(x_i - \overline{x})^2}}
$$

```{r}
Erro.inc <- sqrt(
  sum(
    (iris$Petal.Length - predito)^2
    )/ (150-2) ) / sqrt(sum((iris$Petal.Width - mean(iris$Petal.Width))^2))
...
data.frame(Intercepto=intercepto,Inclinação =inclinação, R2 = R2) %>%  kbl(caption = "Resultados da Regressão linear", digits=16) %>%
